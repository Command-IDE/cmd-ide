...
// DBSchema is the full return value of Read.
...
// Read opens a SQLite file and returns its tables, columns, and up to 500 rows per table.
func Read(path string) (DBSchema, error) {
	...
	for _, tname := range tableNames {
		...
		schema.Tables = append(schema.Tables, DBTable{
			Name: tname, Columns: cols, Rows: tableRows, RowCount: rowCount,
		})
	}
	return schema, nil
}
