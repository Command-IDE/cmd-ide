...
// CollectEntries walks dir and returns all non-hidden files, skipping
// common build artefacts (node_modules, .git, dist, build, vendor, __pycache__).
func CollectEntries(dir string) ([]PackEntry, error) {
...
func CreateZip(dir, zipPath string, entries []PackEntry) error {
	...
	for _, e := range entries {
		...
		fh := &zip.FileHeader{Name: e.RelPath, Method: zip.Deflate}
